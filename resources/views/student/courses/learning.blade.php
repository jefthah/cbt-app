...
<body class="font-poppins text-[#0A090B]">
    <section id="content">
        ...
        <form method="POST" action="{{ route('dashboard.learning.course.answer.store', ['courses' => $course->id, 'questions' => $question->id]) }}" class="learning flex flex-col gap-[50px] items-center mt-[50px] w-full pb-[30px]">
            @csrf
            ...
            <div class="flex flex-col gap-[30px] max-w-[750px] w-full">
                ...
                <label for="answer-{{ $answer->id }}" class="group flex items-center justify-between rounded-full w-full border border-[#EEEEEE] p-[18px_20px] gap-[14px] transition-all duration-300 has-[:checked]:border-2 has-[:checked]:border-[#0A090B]">
                    ...
                    <input type="radio" name="system_answer_id" id="answer-{{ $answer->id }}" value="{{ $answer->id }}" class="hidden">
                </label>
                @endforeach
            </div>
